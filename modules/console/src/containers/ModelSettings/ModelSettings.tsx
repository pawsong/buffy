...
const styles = require('./ModelSettings.css');
...
interface HandlerProps extends ApiDispatchProps, SagaProps {
  modelId: string;
...
@connectApi<HandlerProps>((state, props) => ({
  model: get(`${CONFIG_API_SERVER_URL}/files/${props.modelId}`),
}))
@saga({
...
class SettingsHandler extends React.Component<HandlerProps, HandlerState> {
  ...
  handleModelRefreshRequest = () => this.props.request(this.props.model);
  ...
  handleChangeNameRequest = () => {
    ...
    this.props.runSaga(this.props.changeName, model.id, this.state.filename, this.handleModelRefreshRequest);
  }

  handleChangeVisibilityRequest = () => {
    const model = this.props.model.result;
    this.props.runSaga(this.props.changeVisibility, model.id, !model.isPublic, this.handleModelRefreshRequest);
  }
