...
export default function initCanvas(container: HTMLElement, store: Store, canvasShared: CanvasShared) {
  ...
  const unsubscribe = observeStore(store, state => state.voxelEditor.tool, ({ type }) => {
    if (tool) {
      tool.onLeave();
    }
    tool = tools[type];
