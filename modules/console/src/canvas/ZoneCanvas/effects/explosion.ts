...
if (__CLIENT__) {
  ...
  SPE = require('shader-particle-engine/build/SPE');
}

const explosionSpriteUrl = require('file!./assets/sprite-explosion2.png');
const smokeParticleUrl = require('file!./assets/smokeparticle.png');

function createExplosion(scene, maxAge, position) {
  const loader = new THREE.TextureLoader();
  loader.setCrossOrigin('anonymouse');

  const group = new SPE.Group( {
    texture: {
      value: loader.load(explosionSpriteUrl),
  ...
  const shockwaveGroup = new SPE.Group( {
    texture: {
      value: loader.load(smokeParticleUrl),
    },
    depthTest: false,
    depthWrite: true,
